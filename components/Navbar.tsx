...
export default function Navbar() {
    ...
    return (
        <>
            <motion.nav
                style={{ opacity }}
                className={`fixed top-0 left-0 w-full z-40 h-11 flex items-center justify-between px-6 md:px-8 bg-[rgba(6,6,6,0.78)] backdrop-blur-md border-b border-white/5 ${pathname !== '/' ? 'opacity-100' : ''}`}
            >
                ...
                <div className="flex items-center gap-3 md:gap-4">
                    <div style={{ padding: '1px' }} className="hidden md:block rounded-full bg-gradient-to-r from-[#004FD4] to-[#0077FF] group cursor-pointer hover:shadow-[0_0_15px_rgba(0,119,255,0.4)] transition-shadow">
                    ...
                    <button
                        className="md:hidden text-white/85 focus:outline-none p-1 ml-1"
            ...
            <AnimatePresence>
                {menuOpen && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -20 }}
                        className="fixed inset-0 z-30 pt-[100px] bg-[#060606]/98 backdrop-blur-2xl flex flex-col items-center gap-8 md:hidden"
                    >
                        {NAV_ITEMS.map((item) => (
                            <Link
                                key={item.label}
                                href={item.href}
                                className="text-white/80 hover:text-white text-xl tracking-[0.1em] uppercase font-medium transition-colors"
                                onClick={() => setMenuOpen(false)}
                            >
                                {item.label}
                            </Link>
                        ))}
                        <Link href="/order" onClick={() => setMenuOpen(false)} className="mt-8 text-[15px] font-medium px-10 py-3 rounded-full bg-[#004FD4] text-white hover:bg-[#005aed] transition-colors shadow-[0_4px_14px_0_rgba(0,79,212,0.39)]">
                            Explore the Camera
                        </Link>
                    </motion.div>
                )}
